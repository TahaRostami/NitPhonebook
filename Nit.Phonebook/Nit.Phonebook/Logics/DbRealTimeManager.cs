        ...
        public void Run(object obj)
        {
            ...
            while (con)
            {
                try
                {
                    ...
                    if (ActionInsert)
                    {
                        var tmp = db.ChangeTracingkInformations.Single(r => r.TableName == manager.TabelName && r.Action == "Insert");
                    ...
                    if (ActionUpdate)
                    {
                        lastDtUpdate = db.ChangeTracingkInformations.Single(r => r.TableName == manager.TabelName && r.Action == "Update").LastTime;
                    ...
                    if (ActionDelete)
                    {
                        var tmp = db.ChangeTracingkInformations.Single(r => r.TableName == manager.TabelName && r.Action == "Delete");
